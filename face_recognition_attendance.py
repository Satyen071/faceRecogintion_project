...
class Session:
    ...
    def __init__(self, face_encodings_path, unknwon_face_store_location, face_location_model='hog',
                 face_encoding_model='small', time_between_entry_exit=300):
    ...
    def connect_mysql_database(self, server: str, username: str, password: str, database: str,
                               personnel_table_name: str, attendance_table_name: str,
                               mysql_unknown_name_table: str) -> tuple:
    ...
    def make_known_entry(self, student_id: int, student_university: str, date, time):
        self.__cursor.execute(
            f"SELECT entry_exit FROM {self.__mysql_attendance_table} WHERE student_id = %s AND university = %s AND date = %s",
            (int(student_id), student_university, date))
        ...
        if entry_exit is None:
            ...
            if self.checking_time_interval(self.__time_between_entries,
                                           last_entry):  # Only execute if desired time interval in seconds have passed between last entry and now
    ...
    def start_session(self, camera_index=0, show_preview=True, scale_frame=0.77, desired_fps=2, tolerance=0.45):
        ...
        try:
            ...
            while True:
                ...
                rgb_frame = cv2.cvtColor(small_frame,cv2.COLOR_BGRA2BGR)
                ...
                print(number_of_faces_detected)
                ...
                if show_preview == True:
                    ...
                    for (top, right, bottom, left), identity in zip(face_locations, self.__identified_student_ids):
                        ...
                        cv2.putText(small_frame, identity, (left, bottom + 20), cv2.FONT_HERSHEY_DUPLEX, 0.5,
                                    (0, 255, 0), 1)
