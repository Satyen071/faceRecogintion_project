...
from dbConnector import connect_mysql_database
from db_setup import setup_mysql_db
from populate_personal_table import make_personnel_entry
from face_recognition_attendance import Session




# Path to files
stored_face_encodings_path = './multiple_angles_faces_encodings'
...
session = Session(
    ...
    face_location_model='cnn', #'cnn' has better accuracy but uses GPU, 'hog' is faster with less accuracy uses cpu
    face_encoding_model = 'large', #'large' model has better accuracy but is slower, 'small' model is faster
